...
data_path = os.path.join(base_dir, 'results', 'attack_outputs')
output_path = os.path.join(base_dir, 'results', 'MADGAN_Data')

os.makedirs(output_path, exist_ok=True)

AllPatientsData = joblib.load(os.path.join(data_path,'adversarial_data.pkl'))
...
np.save(os.path.join(output_path, 'sepsis_train_least_0.npy'), AllPatientsData[AllPatientsData['PatientID'].isin(LessVulnerablePatientIDs)].drop(columns=['PatientID']).to_numpy())
np.save(os.path.join(output_path, 'sepsis_train_most_0.npy'), AllPatientsData[AllPatientsData['PatientID'].isin(MoreVulnerablePatientIDs)].drop(columns=['PatientID']).to_numpy())
np.save(os.path.join(output_path, 'sepsis_train_all_0.npy'), AllPatientsData[AllPatientsData['PatientID'].isin(LessVulnerablePatientIDs)].drop(columns=['PatientID']).to_numpy())
...
for run in range(5):
    ...
    np.save(os.path.join(output_path, 'sepsis_train_samples_'+str(run)+'.npy'), AllPatientsData[AllPatientsData['PatientID'].isin(train_indices)].drop(columns=['PatientID']).to_numpy())


cv=0
kf = KFold(n_splits=5)
for train_indices, test_indices in kf.split(AllPatientIDs):
    np.save(os.path.join(output_path, 'sepsis_test_all_' + str(cv) + '.npy'), AllPatientsData[AllPatientsData['PatientID'].isin([AllPatientIDs[i] for i in test_indices])].drop(columns=['PatientID']).to_numpy())
    cv+=1
